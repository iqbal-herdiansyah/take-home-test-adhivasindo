import type { Column, Task, TaskChecklist } from "@/data/column/interface";
...
interface UpdateTaskModalProps {
	...
	onUpdateTask: (payload: TaskSchema) => void;
...
const UpdateTaskModal: React.FC<UpdateTaskModalProps> = ({
	...
	const { columns: columnsData } = useColumn();
	const { users } = useUser();
	...
	const handleSave = useCallback(
		(event: FormEvent) => {
			...
			onUpdateTask(taskForm.getValues());
			onDiscard();
	...
	const handleDelete = useCallback(() => {
		onDeleteTask(selectedTaskId as string);
		onDiscard();
	...
	}, [selectedTaskId, presentToast]);

	useEffect(() => {
		const tasks = Object.entries(columnsData).flatMap((t) => t[1]) as Task[];
		const task = tasks.find((t) => t.id === selectedTaskId);

		if (!task) return;

		taskForm.setValue("id", task.id);
		taskForm.setValue("title", task.title);
		taskForm.setValue("description", task.description);
		taskForm.setValue("assignees", task.assignees as any);
		taskForm.setValue("due_date", task.due_date);
		taskForm.setValue("label", task.label);
		taskForm.setValue("priority", task.priority as any);
		taskForm.setValue("checklist_items", task.checklist_items);
		taskForm.setValue("attachment_files", task.attachment_files);
		taskForm.setValue("cover_image", task.cover_image);
	}, [selectedTaskId, columnsData]);

	return (
		<div className="update-task-modal">
			<form onSubmit={handleSave} className="update-task-modal-content">
				<div className="update-task-modal-left">
					<div className="update-task-modal-left-content">
				...
				<div className="update-task-modal-right">
					<div className="update-task-modal-right-content">
								...
								render={({ field }) => (
									<>
										...
										{isShowChecklistInput && (
											<div className="update-new-checklist-section">
												<input
													value={checklistText}
					...
					<div className="update-task-modal-actions">
						<div className="delete-task-section">
							<IonButton
								...
								className="update-task-modal-discard-button"
							>
								Delete
							</IonButton>
						</div>

						<IonButton type="button" color="medium" onClick={onDiscard} className="update-task-modal-discard-button">
							Discard
						</IonButton>

						<IonButton type="submit" className="update-task-modal-save-button">
							Save
						</IonButton>
					</div>
				</div>
